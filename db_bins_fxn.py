...
def add_bin(username, bin, bin_datestamp, bin_level, bin_completion_date, bin_status):
    c.execute('INSERT INTO bins_table(username, bin, bin_completion_date, bin_status) VALUES ({},{},{},{})'.format(username, bin, bin_completion_date, bin_status))
    c.execute('INSERT INTO bin_dates_table(bin_datestamp, bin_level, username, bin) VALUES ({},{},{},{})'.format(bin_datestamp, bin_level, username, bin))
...
def view_all_bins_details(username):
    c.execute('SELECT bin FROM bins_table WHERE username = {}'.format(username)) # no need for DISTINCT
    data = c.fetchall()
    return data
...
def update_bin_details(new_bin, new_bin_completion_date, new_bin_status, username, bin):
    c.execute('UPDATE bins_table SET bin = {}, bin_completion_date = {}, bin_status = {} WHERE username = {} and bin = {}'.format(new_bin, new_bin_completion_date, new_bin_status, username, bin))
    data = c.fetchall()
    return data
...
def delete_bins(username, bin):
    c.execute('DELETE FROM bin_dates_table WHERE bin = {} AND username = {}'.format(bin, username))
    c.execute('DELETE FROM bins_table WHERE bin = {} AND username = {}'.format(bin, username))
