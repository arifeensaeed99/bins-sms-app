    ...
    if choice == 'About':
        ...
        if log_in:
                ...
                if result:                                   
                        ...
                        year = datetime.datetime.now(pytz.timezone(user_tz)).strftime("%Y")

                        month = datetime.datetime.now(pytz.timezone(user_tz)).strftime("%B")

                        day = datetime.datetime.now(pytz.timezone(user_tz)).strftime("%d")

                        weekday = datetime.datetime.now(pytz.timezone(user_tz)).strftime("%A")
                        ...
                        if username != st.session_state['current_user']: # first login
                                ...
                                with st.empty():
                                        ...
                                        user_time = datetime.datetime.now(pytz.timezone(user_tz)).strftime("%m/%d/%Y at %I:%M:%S %p")
                        ...
                        if st.session_state['current_user'] == username:
                                ...
                                if activity == 'Report Current Levels':
                                    ...
                                    stamps = st.checkbox('Select this box if you want to see trends based on dates and date timestamps. Otherwise, only dates will appear as options', True)
                                    if stamps:
                                        date_type = 'Datestamp'
                                    else:
                                        date_type = 'Date'
                                    ...
                                    if bin:
                                        ...
                                        if st_datestamp < end_datestamp:
                                            ...
                                            st.warning('Please ensure start date is before end date or is not equal to end date')
                                            st.write("Note: If this is the first day of the Bin, more data is needed for other days before trends appear")
                                    ...
                                    if selected_result:
                                        ...
                                        if new_bin:
                                            ...
                                            # UTC Heroku to local user timezone, Time Zone conversion here for closeness to true time
                                            current_datetime = datetime.datetime.now(pytz.timezone(user_tz)).strftime("%Y-%m-%d %H:%M:%S")
                                                                    ...
                                                                    if new_phone:
                                                                        ...
                                                                        "Ting", "U.S. Cellular", "Verizon", "Virgin Mobile" ])
                                                                                    if new_carrier:
                                                                                        ...
                                                                                        t.sort(reverse = True)
                                                                                        new_timezone = st.selectbox("Please select a timezone:", t) # list of all timezeones
                                                                                        if new_timezone:    
        ...
        if current_username and current_password:
            if result:
                ...
                if st.checkbox('Edit Timezone'):
                    ...
                    t = list(pytz.all_timezones_set)
                    t.sort(reverse = True)  
                    timezone = st.selectbox("Please select a new timezone:", t) 
                    if timezone:
